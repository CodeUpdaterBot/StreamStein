...
from utils import (
    apply_browser_impersonation,
    apply_player_headers_opts,
    apply_videasy_headers_opts,
    curl_cffi_available,
    format_filesize,
    friendly_error,
    get_player_header_profiles,
    get_videasy_headers,
    is_access_denied_error,
    is_cloudflare_error,
    is_cookie_db_error,
    sanitize_filename,
    should_retry_with_access_fallback,
)
...
def _access_fallback_attempts(base_opts):
    """Ordered yt-dlp option transforms for CDN access (impersonation is already default).

    Prefer Electron-provided / player Referer-Origin first — anonymous generic fetches
    often 403 on stream CDNs. Impersonation-only ("default") is last resort.
    """
    attempts = []
    existing = (base_opts or {}).get("http_headers") or {}
    if existing.get("Referer") or existing.get("Origin"):
        attempts.append(("provided headers", lambda o: dict(o)))

    attempts.append(("videasy headers", apply_videasy_headers_opts))
    for profile in get_player_header_profiles():
        if profile.get("_label") == "videasy":
            continue
        label = f"{profile.get('_label', 'player')} headers"
        attempts.append(
            (label, lambda o, p=profile: apply_player_headers_opts(o, p))
        )

    def _without_player_headers(o):
        opts = dict(o or {})
    ...
    attempts.append(("default", _without_player_headers))
    return attempts


def _extract_info_with_access_fallbacks(url, base_opts):
    last_err = None
    attempts = _access_fallback_attempts(base_opts)
    for index, (label, transform) in enumerate(attempts):
        opts = transform(base_opts)
        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
                return ydl.extract_info(url, download=False), opts, label
        except Exception as e:
            last_err = e
            err_str = str(e)
            has_more = index < len(attempts) - 1
            if not has_more or not should_retry_with_access_fallback(err_str):
                raise
            print(
                f"[retry] {label} failed ({friendly_error(err_str)}), trying next fallback..."
            )
    raise last_err


def _download_with_access_fallbacks(url, base_opts):
    """Download with header/cookie fallbacks.

    Prefer Electron cookiefile for CDN streams. If cookiesfrombrowser is set and
    fails (locked DB *or* 403), retry with cookiefile / headers only.
    """
    opts = dict(base_opts or {})
    fallback_cookiefile = opts.pop("_streamstein_cookiefile_fallback", None)

    try:
        _download_with_access_fallbacks_inner(url, opts)
        return
    except Exception as e:
        err = str(e)
        can_drop_browser = bool(opts.get("cookiesfrombrowser")) and (
            is_cookie_db_error(err) or is_access_denied_error(err)
        )
        if not can_drop_browser:
            raise
        print(
            "[retry] Browser cookies failed "
            f"({friendly_error(err)}); "
            "falling back to Electron session cookies / headers…"
        )
        opts = dict(opts)
        opts.pop("cookiesfrombrowser", None)
        jar = opts.get("cookiefile") or fallback_cookiefile
        if jar:
            opts["cookiefile"] = jar
        _download_with_access_fallbacks_inner(url, opts)


def _download_with_access_fallbacks_inner(url, base_opts):
    last_err = None
    attempts = _access_fallback_attempts(base_opts)
    for index, (label, transform) in enumerate(attempts):
        opts = transform(base_opts)
        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
                ydl.download([url])
            if index > 0:
                print(f"[retry] Download succeeded with {label}.")
            return
        except Exception as e:
            last_err = e
            err_str = str(e)
            if is_cookie_db_error(err_str):
                raise
            has_more = index < len(attempts) - 1
            if not has_more or not should_retry_with_access_fallback(err_str):
                if is_cloudflare_error(err_str) and not curl_cffi_available():
                    raise Exception(
                        "Cloudflare anti-bot protection detected but curl_cffi is not installed. "
                        'Rebuild vid-dl with: pip install "yt-dlp[default,curl-cffi]"'
                    ) from e
                raise
            if is_cloudflare_error(err_str) and not curl_cffi_available():
                raise Exception(
...
def build_ydl_opts(
    ...
    apply_browser_impersonation(ydl_opts)
    return ydl_opts
...
def _download_direct(url, folder, progress_cb=None, size_cb=None, cancelled_cb=None):
    ...
    if _cffi_requests is None:
        raise Exception(
            "curl_cffi is required for direct downloads. "
            'Rebuild vid-dl with: pip install "yt-dlp[default,curl-cffi]"'
        )
    head = _cffi_requests.head(
