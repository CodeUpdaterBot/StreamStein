...
from workers import (
    YTDLPLogger,
    _download_direct,
    _download_with_access_fallbacks,
    ...
    while i < len(extra_args):
        ...
        if arg in VALUE_FLAGS:
            ...
            if key == "cookiesfrombrowser":
                parsed = _parse_cookies_from_browser(val_str)
                if parsed:
                    ydl_opts[key] = parsed
                continue
        ...
        if arg in ("--add-header", "--add-headers"):
            if i + 1 >= len(extra_args):
                print(
...
def _parse_cookies_from_browser(spec: str):
    """Parse yt-dlp --cookies-from-browser value into cookiesfrombrowser tuple.

    Supports BROWSER[+KEYRING][:PROFILE][::CONTAINER], e.g. chrome:Profile 69.
    """
    ...
    if ":" in raw:
        ...
        profile = profile.strip().strip("\"'") or None
    if "+" in browser:
...
def run_cli(argv: Optional[List[str]] = None) -> int:
    ...
    if extra_ytdlp:
        ...
        # Prefer Chrome cookies when both are present; keep jar for lock fallback
        if ydl_opts.get("cookiesfrombrowser") and ydl_opts.get("cookiefile"):
            ydl_opts["_streamstein_cookiefile_fallback"] = ydl_opts.pop(
                "cookiefile"
            )
    ...
    if curl_cffi_available():
        print(
            f"  Browser impersonation: {default_impersonate_target()} "
            "(generic:impersonate=chrome)"
        )
    else:
        print("  Browser impersonation: disabled (curl_cffi not installed)")
    if ydl_opts.get("cookiesfrombrowser"):
        print(f"  Cookies from browser: {ydl_opts['cookiesfrombrowser']}")
    if ydl_opts.get("cookiefile") or ydl_opts.get("_streamstein_cookiefile_fallback"):
        print(
            "  Cookie file:",
            ydl_opts.get("cookiefile")
            or ydl_opts.get("_streamstein_cookiefile_fallback"),
        )
    if ydl_opts.get("http_headers", {}).get("Cookie"):
        print("  Cookie header: present (from live player request)")
    if final_fullpath:
    ...
    # Perform download via yt-dlp (browser impersonation enabled by default;
    # player-header / cookie fallbacks on 403)
    try:
        _download_with_access_fallbacks(url, ydl_opts)
        print("\nDownload finished.")
        return 0
    except Exception as e:
        print("\nDownload failed:", e)
