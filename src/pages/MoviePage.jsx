...
export default function MoviePage({
  ...
  const castLoadArgs = useMemo(() => {
    const castTitle = item.title || item.name || "Streamstein";
  ...
  const handleLocalProgress = useCallback(
    ...
    [progressKey, watchedThreshold],
  );

  // ── Auto-track progress + auto-watched every 5s ──────────────────────────
  useEffect(() => {
    if (!playing || mediaPlayback.isLocalMode) return;
    if (!sourceSupportsProgress(playerSource)) return;
    let interval = null;
  ...
  const movieWebviewSrc = useMemo(() => {
    ...
    return getSourceUrl(playerSource, "movie", item.id, null, null);
  }, [
    pipOpen,
    cast.currentDevice,
    mediaPlayback.isLocalMode,
            ...
            <div className="player-overlay-group">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (await connectPreferredAndLoad(cast, castLoadArgs)) return;
                  setShowCastPicker(true);
                }}
                title={
                  cast.currentDevice
