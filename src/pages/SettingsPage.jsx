...
function BackupRestoreSection({ onRestored }) {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `streamstein-backup-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
...
function LibraryPrivacySection() {
  ...
  const handleSave = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("streamstein:library-sort-changed", { detail: sort }),
    );
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  };
...
function StartPageSection() {
  ...
  return (
    <div className="settings-panel" style={{ marginBottom: 40 }}>
      ...
      >
        Choose which page opens when you launch Streamstein.
      </div>
      <div
        style={{
          display: "flex",
...
function TmdbLanguageSection() {
  ...
  const handleSave = () => {
    ...
    window.dispatchEvent(new CustomEvent("streamstein:tmdb-lang-changed"));
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  };
...
function CastSettingsSection() {
  ...
  const [autoStop, setAutoStop] = useState(
    () => storage.get(STORAGE_KEYS.CAST_AUTO_STOP) ?? false,
  );
  ...
  const toggleAutoStop = (v) => {
    setAutoStop(v);
    storage.set(STORAGE_KEYS.CAST_AUTO_STOP, v);
  };
  const handlePreferred = (id) => {
    setPreferredId(id);
    if (id) storage.set(STORAGE_KEYS.CAST_PREFERRED_DEVICE_ID, id);
    else storage.remove(STORAGE_KEYS.CAST_PREFERRED_DEVICE_ID);
  };
  ...
  const castToggleRow = (value, onChange, title, desc) => (
    <div
      style={{
        padding: 16,
        background: "var(--surface)",
  ...
  return (
    <div className="settings-panel" style={{ marginBottom: 40 }}>
      {castToggleRow(
        ...
        "Streamstein scans the LAN for Chromecast and DLNA receivers so the picker is ready instantly.",
      )}
      {castToggleRow(
        enableDlna,
        toggleDlna,
        "Include DLNA (UPnP) devices",
        "Useful for older smart TVs and non-AndroidTV projectors. Disable if SSDP discovery is noisy on your network.",
      )}
      {castToggleRow(
        autoStop,
        toggleAutoStop,
        ...
        >
          <button
            ...
            onClick={() => {
              setScanning(true);
              window.electron
                ?.castStartDiscovery?.({ enableDlna, force: true })
      ...
      >
        ...
        <ul style={{ margin: 0, paddingLeft: 18 }}>
          ...
          <li>Mid-segment seeks on rewritten HLS can take 2–3 seconds.</li>
...
export default function SettingsPage({
  ...
  const handleSaveDownloadLimit = async () => {
    const val = Math.max(1, Math.min(20, Number(downloadMaxConcurrent) || 3));
    setDownloadMaxConcurrent(val);
    storage.set(STORAGE_KEYS.DOWNLOAD_MAX_CONCURRENT, val);
    if (window.electron?.setMaxConcurrentDownloads) {
      await window.electron.setMaxConcurrentDownloads(val);
    }
    ...
    <>
      ...
      {showDeleteDlConfirm && (
        <ConfirmDialog
          ...
          description="This will permanently delete all video files downloaded through Streamstein and remove them from the download list."
      ...
      >
        ...
        <div ref={secDownloads} style={{ scrollMarginTop: 80 }}>
          ...
          <div className="settings-panel" style={{ marginBottom: 40 }}>
            ...
            >
              Maximum number of downloads that run at the same time. If you start
              more than this limit (for example every episode in a season),
              extra items wait in a queue and begin automatically when a slot
              opens. Helps avoid overwhelming streaming servers. Default is 3;
            ...
            {downloadLimitSaved && (
              <div style={{ marginTop: 10, fontSize: 13, color: "#48c774" }}>
                ✓ Saved — new limit applies immediately
              </div>
            )}
          </div>
        </div>
          ...
          >
            ...
            <div style={{ padding: "22px 24px" }}>
              <CleanRow
                ...
                description="Opens the folder where Streamstein is installed."
            ...
            <div style={{ padding: "22px 24px" }}>
              <CleanRow
                title="Delete All Downloads"
                description="Permanently deletes all video files that were downloaded through Streamstein and removes them from the download list. Only files downloaded through the app will be deleted, nothing else in your folder is touched."
            ...
            >
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  >
                    Completely resets Streamstein to factory defaults, clears all
