...
import { runTvSeriesPlayerDownload } from "../utils/tvSeriesPlayerDownload";
...
import { connectPreferredAndLoad } from "../utils/castUtils";
...
export default function TVPage({
  ...
  /** @type {import('react').MutableRefObject<null | { wcId: number, urls: string[], settleTimer: any, resolve: (v: string|null) => void }>} */
  const seriesBatchRef = useRef(null);
  const playingRef = useRef(false);
  const webviewLoadingRef = useRef(false);
  const playerSourceRef = useRef(playerSource);
  playingRef.current = playing;
  webviewLoadingRef.current = webviewLoading;
  playerSourceRef.current = playerSource;
  ...
  /** Flat episode list for batch download ({ season, episode, uiSeason }). */
    ...
    if (episodeGroupData?.groups) {
      ...
      sortedGroups.forEach((g, i) => {
        ...
        [...(g.episodes || [])]
          .sort((a, b) => a.order - b.order)
          .forEach((ep, idx) => {
            const mapped = applyEpisodeMapping(
              item.id,
              ep.season_number,
              ep.episode_number,
              episodeGroupMap,
    ...
    for (const s of seasons) {
      ...
      for (let ep = 1; ep <= count; ep++) {
        list.push({
          ...
          episode: ep,
          uiSeason: s.season_number,
          uiEpisode: ep,
        });
      }
    }
    return list;
  }, [episodeGroupData, seasons, item.id, episodeGroupMap]);
  ...
  const tvPlaybackProgressKey = selectedEp
    ? `tv_${item.id}_s${selectedSeason}e${selectedEp.episode_number}`
    : `tv_${item.id}_pending`;

  const mediaPlayback = useMediaPlaybackSource({
    playing: playing && !!selectedEp,
    progressKey: tvPlaybackProgressKey,
    mediaType: "tv",
    tmdbId: item.id,
  ...
  const castLoadArgs = useMemo(() => {
    const baseTitle = item.name || item.title || "Streamstein";
    const epLabel = playerEp
      ? ` · S${playerEp.season}E${playerEp.episode}`
      : "";
    const castTitle = `${baseTitle}${epLabel}`;
    const posterUrl = d.poster_path ? imgUrl(d.poster_path, "w500") : null;
    const subs = interceptedSubs.map((s) => ({ url: s.url, lang: s.lang }));
    if (mediaPlayback.isLocalMode && mediaPlayback.localPath) {
      const localSubs = (mediaPlayback.localDownload?.subtitlePaths || [])
        .filter((sp) => sp?.path)
      ...
      return {
        ...
        title: castTitle,
        posterUrl,
        startTime: storage.get("dlTime_" + tvPlaybackProgressKey) || 0,
        localVttSubs: localSubs,
      };
    }
    if (isAsync && m3u8Url) {
    ...
    tvPlaybackProgressKey,
  ]);

  useEffect(() => {
    if (playing || !cast.currentDevice) return;
    if (!storage.get(STORAGE_KEYS.CAST_AUTO_STOP)) return;
    cast.disconnect();
  }, [playing, cast.currentDevice, cast]);
  ...
  useEffect(() => {
    ...
    const onFail = (e) => {
      ...
      if (mediaPlayback.isLocalMode) {
        mediaPlayback.reportLocalPlaybackError();
    ...
    selectedEp?.episode_number,
    mediaPlayback.isLocalMode,
    mediaPlayback.localPlayerUrl,
    mediaPlayback.reportLocalPlaybackError,
  ]);

  const handleLocalProgress = useCallback(
    ({ currentTime, duration }) => {
      if (!currentProgressKey || !duration || duration <= 0) return;
      const ct = currentTime;
      lastKnownTimeRef.current = ct;
      durationRef.current = duration;
      const p = Math.floor((ct / duration) * 100);
    ...
    [currentProgressKey, watchedThreshold],
  );
  ...
  useEffect(() => {
    ...
    if (!playing || !currentProgressKey || mediaPlayback.isLocalMode) return;
  ...
  const tvWebviewSrc = useMemo(() => {
    if (pipOpen || cast.currentDevice) return "about:blank";
    if (mediaPlayback.isLocalMode) {
      return "about:blank";
    }
    if (isAsync) {
      return resolvedPlayerUrl || "about:blank";
    }
    return getSourceUrl(
      playerSource,
      "tv",
  ...
  const beginBatchCapture = useCallback(() => {
    seriesBatchRef.current = null;
  }, []);

  const endBatchCapture = useCallback(() => {
    seriesBatchRef.current = null;
  }, []);

  const cancelBatchM3u8 = useCallback(() => {
    const batch = seriesBatchRef.current;
    if (batch) {
      clearTimeout(batch.settleTimer);
      batch.resolve(null);
  ...
  const peekBatchM3u8 = useCallback(() => {
    const batch = seriesBatchRef.current;
    if (!batch?.urls?.length) return null;
    return pickBestMediaUrl(batch.urls);
  }, []);

  const waitForBatchM3u8 = useCallback((wcId, signal) => {
    return new Promise((resolve) => {
      const finish = (url) => {
        clearTimeout(seriesBatchRef.current?.settleTimer);
        seriesBatchRef.current = null;
        resolve(url || null);
      };
      const batch = {
      ...
      seriesBatchRef.current = batch;
      const maxTimer = setTimeout(() => finish(pickBestMediaUrl(batch.urls)), 90000);
      const onAbort = () => {
        clearTimeout(maxTimer);
        finish(pickBestMediaUrl(batch.urls));
      };
      signal?.addEventListener("abort", onAbort, { once: true });
      const origResolve = finish;
      batch.resolve = (url) => {
        clearTimeout(maxTimer);
  ...
  const loadEpisodeInPlayer = useCallback(
    (ep, { signal, sourceId } = {}) => {
      if (sourceId && sourceId !== playerSourceRef.current) {
        playerSourceRef.current = sourceId;
        setPlayerSource(sourceId);
        storage.set("playerSource", sourceId);
      }

      const expectedUrl = getSourceUrl(
        sourceId || playerSourceRef.current,
        "tv",
        item.id,
        ep.season,
        ep.episode,
      ...
      const expectedHost = (() => {
        try {
          return new URL(expectedUrl).hostname;
        } catch {
          return "";
        }
      })();
      ...
      return new Promise((resolve) => {
        ...
        setResolvingUrl(false);
        setResolveError(null);
        setSelectedSeason(ep.uiSeason ?? ep.season);
        setSelectedEp({
          episode_number: ep.uiEpisode ?? ep.episode,
          name: `Episode ${ep.uiEpisode ?? ep.episode}`,
          _tmdbSeason: ep.season,
          _tmdbAbsolute: ep.episode,
        });
        setPlaying(true);
        ...
        const timeout = setTimeout(
          () => resolve({ ok: false, error: "Player load timed out" }),
          90000,
        );
        const iv = setInterval(() => {
          ...
          const urlReady =
            !expectedHost || currentUrl.includes(expectedHost);
          if (
            playingRef.current &&
            !webviewLoadingRef.current &&
            wcId &&
            wcId > 0 &&
            urlReady
          ) {
  ...
  const runTvSeriesBatch = useCallback(
    async (job, deps) => {
      return runTvSeriesPlayerDownload(
        {
          beginBatchCapture,
          endBatchCapture,
          cancelBatchM3u8,
          ...
          applyPlayerSource: (id) => {
            playerSourceRef.current = id;
            setPlayerSource(id);
    ...
    <div className="fade-in">
              ...
              <div className="detail-info">
                ...
                <div className="detail-actions">
                  ...
                  <button
                    ...
                    title="Automatically resolve and queue all episodes"
                  >
            ...
            <div className="section">
              ...
              >
                ...
                <div className="player-overlay-group">
                  <PlaybackModeToggle
                    mode={mediaPlayback.effectiveMode}
                    canToggle={mediaPlayback.canToggle}
                    onSetMode={(mode) => {
                      mediaPlayback.setPlaybackMode(mode);
                      if (mode === "stream") setWebviewLoading(true);
                    }}
                  />
                  <button
                    ref={sourceRef}
                    className="player-overlay-btn"
                    style={
                      mediaPlayback.isLocalMode ? { display: "none" } : undefined
                    }
                    onClick={() => {
                  ...
                  <button
                    ...
                    onClick={async () => {
                      setShowSourceMenu(false);
                      if (cast.currentDevice) {
                        setShowCastPicker(true);
                        return;
                      }
                      if (await connectPreferredAndLoad(cast, castLoadArgs))
                        return;
                      setShowCastPicker(true);
                    }}
                    title={
                    ...
                    onClick={() => {
                      ...
                      const openPip = async () => {
                        if (
                          ...
                          pipUrlRef.current = mediaPlayback.localPlayerUrl;
                  ...
                  title={
                    currentEpDownload
                      ? currentEpDownload.status === "downloading" ||
                        currentEpDownload.status === "queued" ||
                        currentEpDownload.status === "resolving"
                ...
                {/* AniSkip manual prompt, rendered in Streamstein UI, outside webview */}
                {skipPrompt && (
                  <button
      ...
      {showSeriesDownload && (
        <SeriesDownloadModal
          ...
          runSeriesDownload={runTvSeriesBatch}
        />
      )}
        ...
        >
          ...
          {epDownload && (
            <span
              ...
              title={
                epDownload.status === "resolving"
                  ? "Fetching stream link - click to view in Downloads"
                  : epDownload.status === "queued"
                    ? "Queued - click to view in Downloads"
