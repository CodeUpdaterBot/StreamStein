...
export default function App() {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      let val = await secureStorage.get("apikey");
      // Fallback: older Streambert builds or failed OS re-encryption may leave
      // the token only in localStorage (streambert_apikey → streamstein_apikey).
      if (!val) {
        const fromStorage = storage.get(STORAGE_KEYS.API_KEY);
        if (fromStorage) {
          await secureStorage.set("apikey", fromStorage);
          val = fromStorage;
        }
      }
  ...
  // Sync download concurrency limit to main process on startup / when settings change
  useEffect(() => {
    if (!window.electron?.setMaxConcurrentDownloads) return;
    const max =
      storage.get(STORAGE_KEYS.DOWNLOAD_MAX_CONCURRENT) ?? 3;
    window.electron.setMaxConcurrentDownloads(max);
  }, []);

  // Active download count for sidebar badge (running + queued)
  ...
  useEffect(() => {
    ...
    window.addEventListener("streamstein:library-sort-changed", handler);
    return () =>
      window.removeEventListener("streamstein:library-sort-changed", handler);
  }, []);

  // ── Re-fetch trending immediately when metadata language changes ──────────
  useEffect(() => {
    const handler = () => fetchTrending();
    window.addEventListener("streamstein:tmdb-lang-changed", handler);
    return () =>
  ...
  const handleShowAllLibrary = useCallback(() => {
    navigate("downloads");
  }, [navigate]);

  const handleOpenUntrackedLocal = useCallback(
    (item) => {
      setHighlightDownload(item?._downloadId || null);
      navigate("downloads");
    },
    [navigate],
  );
    ...
    <>
      ...
      <div>
          ...
          >
            {page === "home" && (
              <HomePage
                ...
                apiKey={apiKey}
                downloads={downloads}
                onShowAllLibrary={handleShowAllLibrary}
                onOpenUntrackedLocal={handleOpenUntrackedLocal}
              />
            )}
            {page === "movie" && selected && (
              <MoviePage
        ...
        {updateBanner && (
          ...
          >
            <span>🎉 Streamstein v{updateBanner.latest} is available!</span>
            <button
  ...
  return (
    <ErrorBoundary>
      {isElectron ? (
        <StreamCaptureProvider>{appBody}</StreamCaptureProvider>
      ) : (
        appBody
      )}
    </ErrorBoundary>
