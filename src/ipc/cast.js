...
// dlnacasts3 logs `[DLNACASTS] …` via console.debug on every SSDP query/response (async).
const _origConsoleDebug = console.debug.bind(console);
console.debug = (...args) => {
 const first = args[0];
 if (typeof first === "string" && first.includes("[DLNACASTS]")) return;
...
let _dlnaBurstTimers = [];

// ── Helpers ─────────────────────────────────────────────────────────────────

function _emitDevicesNow() {
...
function pushDevicesUpdate(immediate = false) {
 if (immediate) {
...
/** Rewrite an HLS playlist so every segment URL flows through /proxy/. */
...
// Passive rescans (auto-discover on page load) — keep SSDP polite on the LAN.
const DLNA_SSDP_THROTTLE_MS = 2500;
// Picker open / Refresh bypasses throttle and runs extra SSDP + mDNS passes.
const DLNA_BURST_DELAYS_MS = [900, 2200, 4500];

function _clearDlnaBurst() {
 for (const t of _dlnaBurstTimers) clearTimeout(t);
 _dlnaBurstTimers = [];
}

function _scheduleDlnaBurst() {
 _clearDlnaBurst();
 if (!_dlnaList) return;
 for (const delay of DLNA_BURST_DELAYS_MS) {
 const t = setTimeout(() => {
 if (!_dlnaList) return;
 _dlnaList.update();
 pushDevicesUpdate(true);
 }, delay);
 if (typeof t.unref === "function") t.unref();
 _dlnaBurstTimers.push(t);
 }
}

function _scheduleCastMdnsBurst() {
 if (!_bonjourBrowser) return;
...
async function startDiscovery({ enableDlna = true, force = false } = {}) {
 ...
 if (force) _scheduleCastMdnsBurst();
 } catch (e) {
 ...
 const now = Date.now();
 if (force || now - _lastDlnaUpdate >= DLNA_SSDP_THROTTLE_MS) {
 _lastDlnaUpdate = now;
 _dlnaList.update();
 }
 if (force) _scheduleDlnaBurst();
 } catch (e) {
...
function register(getMainWindow) {
 ...
 // Warm discovery in the background so Cast/DLNA devices are often cached
 // before the user opens the picker (LG TVs, Sonos, Chromecast, etc.).
 setTimeout(() => {
 startDiscovery({ enableDlna: true, force: false }).catch(() => {});
 }, 1200).unref?.();
