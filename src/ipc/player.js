...
const os = require("os");
const { triggerEmbedPlayback } = require("./embedAutomation");
...
function register(getMainWindow, { writeSecretMigration }) {
  ...
  ipcMain.handle("download-and-install-update", async (_, { url, format }) => {
    try {
      ...
      const ext =
        ...
        : format === "dmg" ? ".dmg"
        : ".AppImage";
      const destPath = path.join(os.tmpdir(), `streamstein-update${ext}`);
      ...
      await new Promise((resolve, reject) => {
        ...
        const doRequest = (reqUrl, redirectDepth = 0) => {
          ...
          const req = lib.get(
            ...
            {
              headers: {
                "User-Agent": "Streamstein-AutoUpdater",
      ...
      if (format === "appimage") {
        ...
        if (currentAppImage) {
          const scriptPath = path.join(os.tmpdir(), "streamstein-update.sh");
  ...
  // ── Automate embed play controls (series batch + hidden player webview) ───
  ipcMain.handle("automate-embed-player", async (_, webContentsId) => {
    try {
      const { webContents } = require("electron");
      const wc = webContents.fromId(webContentsId);
      if (!wc || wc.isDestroyed()) return { ok: false };
      const result = await triggerEmbedPlayback(wc);
      return { ok: result.ok, clicks: result.clicks ?? 0 };
    } catch (e) {
      return { ok: false, error: e.message };
    }
  });

  ipcMain.handle("click-embed-play-center", async (_, webContentsId) => {
    try {
      const { webContents } = require("electron");
      const wc = webContents.fromId(webContentsId);
      if (!wc || wc.isDestroyed()) return { ok: false };
